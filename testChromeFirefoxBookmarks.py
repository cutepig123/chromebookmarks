...
def listX(data):
    ...
    for i in children[:NUM_BOOKMARKS]:
        #t = time.strftime("%Y-%m-%d %H:%M:%S", time.gmtime(int(i['date_added'])/1000000))
        t = date_from_webkit(int(i['date_added']))

        #If it is too old, skip
        delta1week = datetime.timedelta(days=NUM_DAYS)
        if t+delta1week < datetime.datetime.now(): break
...
def get_bookmarks(cursor):
	...
	where visit_count>0
    order by dateAdded desc limit %d"""%NUM_BOOKMARKS
    ...
    for row in cursor:
        ...
        #t = time.gmtime(dateAdded/1000000)
        t = datetime.datetime.fromtimestamp(dateAdded/1000000)
         #If it is too old, skip
        delta1week = datetime.timedelta(days=NUM_DAYS)
        if t+delta1week < datetime.datetime.now(): break
